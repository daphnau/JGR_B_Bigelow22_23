...
#######Before using this code, set the working directory to the location of the clones GitHub directory######

wdir = "C:/Users/dafna/Desktop/Data_and_Analysis_Bigelow/JGR_B_Bigelow22_23/Data"
setwd(wdir)
...
custom_headers_ <- c("time", "Average_SWP","sd_swp", "Average_PP",  "sd_pp" , "Average_DF" ,"sd_df", 
                     "swp_sensor_M1", "swp_sensor_M2N", "swp_sensor_m2s", "pp_sensor_P1N", "pp_sensor_P3S",
                     "pp_sensor_P3N", "df_sensor_D1S", "df_sensor_S2S") 

sf_data_2022 <- read.csv('MB_SapFlow_SpeciesAverage_2022_with_individuals.csv', header = FALSE, col.names = custom_headers_, sep = ',',skip = 2)
...
##sap flow### 2023
sf_data_2023 <- read.csv('MB_SapFlow_SpeciesAverage_2023_with_individuals.csv', header = FALSE, col.names = custom_headers_, sep = ',',skip = 2)
#####changing column 1 to date and time####
...
t_test_results <- sf_and_tower_noontime_mo_22_23 %>%
  group_by(Season) %>%
  summarise(
    t_test = list(t.test(mean_sap_flow ~ Year)),  # Perform t-test for each season
    .groups = 'drop'
  ) %>%
  mutate(
    p_value = sapply(t_test, function(x) x$p.value),  # Extract p-value from the t-test
...
New_fig2_ <- fig_2a | fig_2b | fig_2c

New_fig2_

ggsave(
  filename = "New_fig2_.tiff",
  plot = New_fig2_,
  device = "tiff",
...
##########FIGURE 4 CORRLTIONS####
dir()
r2_table<-read.csv('corr_table_.csv', sep = ','   )  
...
data<-read.csv('Weekly_means_sapflow_tower_thermal_allday_.csv', sep = ','   )  
  ...
  for (response in response_variables) {
    for (proxy in proxy_variables) {
      ...
      if (nrow(season_data) > 1) {  # Ensure there are enough data points to calculate correlation
        ...
        cor_test_result <- cor.test(season_data[[response]], season_data[[proxy]])
...
# Loop through each season, response variable, and proxy variable
for (season in unique(subset_stats_2023$Season)) {  # Updated to 2023
  for (response in response_variables) {
    for (proxy in proxy_variables) {
      
      # Subset data for the specific season
      season_data <- subset_stats_2023 %>% filter(Season == season)  # Updated to 2023
...
###figure S2##############corrlations####################
daily_noontime_mo<-read.csv('daily_noontime_mo_byseason_s2_.csv', sep = ','   )  
...
###fig 1 2024-2021###########
dir()
tower_2014_2021 <- read.csv('tower_2014_2021.csv', sep = ',',
                          na.strings = c("-9999", "-9999.000"))
...
####rainfall monsoon vs winter###
# Define winter and monsoon months
winter_months <- c( 11, 12, 1, 2, 3, 4)
monsoon_months <- c(5,6, 7, 8, 9,10)

# Add a new column for the rainfall period (winter or monsoon)
Monthly_rainfall <- Monthly_rainfall %>%
  mutate(rainfall_period = ifelse(Month %in% winter_months, "winter", "monsoon"))

# Sum the rainfall (sum_rain1 + sum_rain2) and group by year and period
Monthly_rainfall_summary <- Monthly_rainfall %>%
  group_by(Year, rainfall_period) %>%
  summarise(total_rainfall = sum(sum_rain_1, na.rm = TRUE)) %>%
  ungroup()


ggplot(Monthly_rainfall_summary, aes(x = factor(Year), y = total_rainfall, fill = rainfall_period)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = c("winter" = "#00BFFF", "monsoon" = "#1E90FF")) +  # Light blue for winter, Dark blue for monsoon
  labs(
    x = "Year",
    y = "Total Rainfall (mm)",
    title = ""
